  ...
  class ResendActionEmail extends React.Component {
    ...
    resendActivationEmailHandler = (memberId) => {
      ...
      if (memberId) {
        ...
        this.resendActivationEmailViewModel.resendActivationEmailOnPage(
          memberId);
      }
    };
    ...
    render() {
      ...
      return (<>
        ...
        <div className='mb-4'>
          ...
          <a className='d-block'>{memberInfo
            ? memberInfo.email
            : 'your_account@gmail.com'}</a>
        </div>
        ...
        {loadingStatus ? <button
          className='btn btn-success' disabled={loadingStatus}>
          <div className='spinner-border text-secondary' role='status'>
            <span className='sr-only'>Loading...</span>
          </div>
        </button> : <ButtonNormal text={t('txt_resend_email')}
                                  onClick={() => {
                                    this.resendActivationEmailHandler(
                                      memberInfo.id);
                                  }}
                                  disabled={!memberInfo} />}
      </>);
