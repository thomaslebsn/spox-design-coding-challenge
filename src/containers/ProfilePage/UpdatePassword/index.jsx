...
const UpdatePassword = observer(class UpdatePassword extends Component {
  ...
  validateInfoBeforeSending = () => {
    if (this.validator.allValid()) {
      if (this.formPropsData[UPDATE_PASSWORD_FIELD_KEY.NEW_PASSWORD] !==
  ...
  render () {
    let successResponse = this.updatePasswordViewModel
      ...
      : null;
    let memberInfo = this.updateGeneralViewModel.memberInfo;
    if (memberInfo) this.formPropsData[UPDATE_PASSWORD_FIELD_KEY.ID] = memberInfo.id;
    ...
    return (<> {!memberInfo ? null : <>
        ...
        <div className='bg-white p-3'>
          <form>
            <div className='row'>
              <div className='col-4'>
                <label className='form-label mb-3' htmlFor='curr_password'>
                      <span
                        className='text-black opacity-75'>Current password</span>
                </label>
                <input type='password'
                ...
                {this.validator.message('password',
                  ...
                  'required|min:6|max:30', { className: 'text-danger' })}
              </div>
              <div className='col-4'>
                <label className='form-label mb-3' htmlFor='new_password'>
                      <span
                        className='text-black opacity-75'>New Password</span>
                </label>
                <input type='password'
                       className='form-control mb-3'
                       id='new_password'
                       onBlur={this.blurringFieldHandler}
                       disabled={this.state.loading}
                       onChange={event => this.handleInputChange('new_password',
                         event.target.value)}
                       name='new_password'
              ...
              <div className='col-4'>
                ...
                <input type='password'
                       ...
                       disabled={this.state.loading}
                       onChange={event => this.handleInputChange(
                         'new_checked_password', event.target.value)}
            ...
            {this.state.loading && successResponse.state ? <button
