...
import { EasiiContentThemeApiService } from "easii-io-web-service-library";
...
export default class ContentThemeStore {
  async fetchContentThemes(callbackOnSuccess, callbackOnError, paginationStep) {
    try {
      ...
      const contentThemeAPIService = new EasiiContentThemeApiService();
  ...
  async saveContentTheme(contentThemeData, callbackOnSuccess, callbackOnError) {
    try {
      ...
      const convertedContentData = ContentThemeModel.convertSubmittedDataToAPIService(
        contentThemeData
      );
      ...
      // Save Content
      const contentThemeService = new EasiiContentThemeApiService();
      let resultContent = false;

      if (
        convertedContentData[ESI_CONTENT_THEME_API_RESPONSE_FIELD_KEY.ID] ==
        undefined
      ) {
        resultContent = await contentThemeService.createContentTheme(
          convertedContentData
        );
      } else {
        resultContent = await contentThemeService.updateContentTheme(
          convertedContentData
        );
      }
  ...
  async deleteContentThemes(ids, callbackOnSuccess, callbackOnError) {
    ...
    try {
      ...
      const repondedDataFromLibrary = await contentThemeService.deleteContentTheme(
        deleteIds
      );
  ...
  async getContentTheme(id, callbackOnSuccess, callbackOnError) {
    ...
    try {
      const contentThemeService = new EasiiContentThemeApiService();
      const repondedDataFromLibrary = await contentThemeService.getContentThemeItem(
        id
      );
